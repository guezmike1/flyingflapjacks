...
def get_team_ids(index):
    ...
    data_str = json.dumps(result)
    game_list_outer = json.loads(data_str)

    game_list = game_list_outer[game_list_outer.keys()[0]]["games"]

    away_points = game_list[index]["away_points"]
    home_points = game_list[index]["home_points"]

    away_team_id = game_list[index]["away"]["id"]
    home_team_id = game_list[index]["home"]["id"]

    away_team_name = game_list[index]["away"]["name"]
    home_team_name = game_list[index]["home"]["name"]
...
def run_game(loops,away_team_id,home_team_id,away_team_name,home_team_name):
    ...
    for x in range(0,loops):
        ...
        if print_summary:
            print "Turnovers: ["+str(turnovers[0])+","+str(turnovers[1])+"]"
            print "Blocks:    ["+str(blocks[0])+","+str(blocks[1])+"]"
            print "FG:        ["+str(fg[0])+","+str(fg[1])+"]"
            print "FGatt:     ["+str(fgatt[0])+","+str(fgatt[1])+"]"
            print "offreb:    ["+str(offreb[0])+","+str(offreb[1])+"]"
        ...
        if print_boxscore:
            print_boxscore_function(box_score_away, box_score_home,away_team_name,home_team_name)
...
def print_boxscore_function(box_score_away, box_score_home,away_team_name,home_team_name):
    firebase_db = firebase.FirebaseApplication('https://nbasort.firebaseio.com/', authentication=None)
    ...
    for shooter in box_score_away:
        ...
        if player_outer is None:
            ...
            box_score_away[shooter][0] = player_outer[player_outer.keys()[0]]
    ...
    for shooter in box_score_home:
        ...
        if player_outer is None:
            ...
            box_score_home[shooter][0] = player_outer[player_outer.keys()[0]]
